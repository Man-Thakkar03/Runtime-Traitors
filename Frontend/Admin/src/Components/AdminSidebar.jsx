...
const AdminSidebar = ({ onClose }) => {
  ...
  return (
    <aside className="fixed top-0 bottom-0 left-0 h-screen w-64 flex flex-col justify-between shadow-lg bg-[#1A1C23] z-50 overflow-hidden">
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-6">
          ...
          <button
            onClick={onClose}
            className="lg:hidden text-white hover:text-gray-300 transition p-2 rounded-lg hover:bg-[#23263A]"
          >
        ...
        {/* Menu Items */}
        <nav className="mt-4 flex flex-col gap-1 flex-1 overflow-y-hidden">
          {menu.map((item) => (
            <Link
              key={item.label}
              to={item.path}
